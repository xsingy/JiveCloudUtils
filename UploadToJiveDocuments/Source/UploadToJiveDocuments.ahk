...
;if there is an UploadToJiveDocuments.cfg file in the same directory, parse it to get any parameter provided in JSON format
...
FileRead, Config, UploadToJiveDocuments.cfg
...
Loop {
      ...
      if 0<8
        {
          ...
          Gui, Add, Text, x10 y40 w160 h30 0x2 , Path and pattern of HTML or TXT to upload into Documents
          ...
          Gui, Add, Text, x10 y160 w160 h30 0x2, When a document exists in destination
      ...
      Gui, Color, FFFFFF
      Gui Font, s10 Bold
      Gui, Add, Text, , Uploading HTML and TXT files matching pattern %filePattern% into documents in group %placeName% 
      ...
      /*************************************************************************************************************************
       *
       *   Get list of documents already existing in place
       *  
       */
       
      wInfo("**********************************" )
      wInfo("*** Getting list of documents already existing in """ . placeName . """")
      ...
      Loop{
          ...
          cURL_command := "CURL -i -u " . userID . ":" . password . " " . (curlExtra = "" ? "" : curlExtra . " ") . """" . jiveInstanceURL . "/api/core/v3/contents?count=" . pageSize . "&startIndex=" . startIndex . "&fields=subject,content,tags,categories,parent&filter=place%28" . placeURI . "%29&filter=type%28document%29"""
          ...
          Loop % jsonObj.list.MaxIndex()
            {
              ...
              existingFiles.Insert( jsonObj.list[A_Index].subject , [uriDecode(jsonObj.list[A_Index].resources.self.ref), jsonObj.list[A_Index].tags, jsonObj.list[A_Index].categories ] )
              wInfo( "  " . uriDecode(jsonObj.list[A_Index].subject) ) 
            }
      ...
      /*************************************************************************************************************************
       *
       *   Upload HTML and TXT content into documents
       *  
       */
      
      wInfo("**********************************" )
      wInfo("*** Uploading HTML and TXT files content into Documents in " . placeName )
      
      Loop, %filePattern%
        {
            ;get file name without extension
            extPosition := RegExMatch(A_LoopFileName,A_LoopFileExt)
            loopFileName := SubStr(A_LoopFileName,1,extPosition-2)
            ;set mime type depending of file extension
            isHTML := RegExMatch(A_LoopFileExt,"i)htm")
            isTXT := RegExMatch(A_LoopFileExt,"i)txt")
            if( (isHTML . isTXT) = "00" )
              {
              wInfo( A_LoopFileName . " is not a TXT or HTML file => skipping")
              continue
              }
            mimeType := (isHTML)?"text/html":"text/plain" 
            ;wInfo( "Mime Type: " . mimeType )
            
            ;prepare file content
            ;read content of TXT or HTML file 
            ;copy it in a temp file so it can be read during the loop
            FileCopy, %A_LoopFileFullPath%, content.tmp, 1
            ;read content into contentString variable
            FileRead, contentString, content.tmp
            ;cleanup
            FileDelete, content.tmp
            ;escaping "" and carriage returns 
            StringReplace, contentString, contentString, ", \", All
            if(isHTML)
            {
              StringReplace, contentString, contentString, `a, \n, All
              StringReplace, contentString, contentString, `r`n, \n, All
              StringReplace, contentString, contentString, `r, \n, All
              StringReplace, contentString, contentString, `n, \n, All
            } else {
              StringReplace, contentString, contentString, `a, \n<br>, All
              StringReplace, contentString, contentString, `r`n, \n<br>, All
              StringReplace, contentString, contentString, `r, \n<br>, All
            ...
            ;check if the document already exists in Jive
            if ( ObjHasKey(existingFiles, loopFileName) ) 
              {
                ;if the option is the update the file, do it, otherwise, skip the file
                if updateExisting {
                  
                  wInfo( loopFileName . " is getting updated with the latest version.")
                  ...
                  contentTags := arrayToString(Arr_concatenate(existingFiles[loopFileName][2],tagsA))
                  contentCategories := arrayToString(Arr_concatenate(existingFiles[loopFileName][3],categoriesA))
                  
                  ;build the json string            
                  contentJson := "{""type"":""document"",""subject"":'" . loopFileName . "',""visibility"":""place"",""parent"":'" . placeURI . "',""content"":{""type"":'" . mimeType . "',""text"":""" . contentString . """},""tags"":" . contentTags . ",""categories"":" . contentCategories . "}"            
                  ...
                  cURL_command := "CURL -X PUT -i -u " . userID . ":" . password . " " . (curlExtra = "" ? "" : curlExtra . " ") . " -F ""json=@cou.tmp;type=application/json"" " . existingFiles[loopFileName][1]
                  CmdRun := Cmd.Exec(ComSpec . " /c cURL.exe " . cURL_command)
                  output := CmdRun.StdOut.ReadAll() 
                  ;wPrint(cURL_command . "`n")
                  ...
                  wInfo( loopFileName . " skipped (already existed).")
              }
            else
              {
                ;if the document does not exists - create it!
                
                wInfo( loopFileName . " does not exist yet and will be created.")
                FileDelete, cou.tmp            
                ...
                if(debug="1")
                    wPrint( "JSON string sent to the server:`n" . contentJson . "`n" )
                FileAppend, %contentJson% , cou.tmp
                cURL_command := "CURL -i -u " . userID . ":" . password . " " . (curlExtra = "" ? "" : curlExtra . " ") . " -F ""json=@cou.tmp;type=application/json"" " . jiveInstanceURL . "/api/core/v3/contents"
                CmdRun := Cmd.Exec(ComSpec . " /c cURL.exe " . cURL_command)
                ;wPrint(cURL_command . "`n")
                output := CmdRun.StdOut.ReadAll()
                if(debug="1")
